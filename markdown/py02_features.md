# py02_features.py 代码解释

## 文件概览

本文件是特征工程模块，负责计算技术指标、动量与价格位置特征、成交量特征、基本面截面排名特征、波动率与风险特征、市场环境特征，并生成标签。
...
```
- **第16行**：导入pandas库，用于数据处理。
- **第17行**：导入numpy库，用于数值计算。
- **第18行**：导入os模块，用于文件和路径操作。
- **第19行**：导入gc模块，用于垃圾回收。
- **第20行**：导入warnings模块，用于处理警告。
...
### 第32-265行：compute_features_for_stock函数
...
def compute_features_for_stock(g: pd.DataFrame) -> pd.DataFrame:
    ...
    return feat_df
```
- **第32-265行**：定义compute_features_for_stock函数，对单只股票的DataFrame计算所有特征。
  - **第33-36行**：函数文档字符串，说明函数的作用和输入数据的要求。
  - **第37-44行**：获取股票的各种价格和成交量数据，并转换为float32类型以节省内存。
  - **第46行**：初始化feats字典，用于存储计算的特征。
  - **第49-53行**：计算不同周期的收益率。
  - **第56-59行**：计算1日对数收益率。
  - **第62-70行**：计算不同周期的移动平均线及其偏差。
  - **第73-77行**：计算均线多头排列信号。
  - **第81-88行**：定义ewma函数，用于计算指数加权移动平均。
  - **第90-97行**：计算MACD指标。
  - **第101-110行**：计算不同周期的RSI指标。
  - **第114-119行**：计算布林带指标。
  - **第122-130行**：计算ATR指标。
  - **第134-142行**：计算KDJ指标。
  - **第146-170行**：计算成交量相关特征，包括成交量比率、OBV、换手率和成交额。
  - **第173-185行**：计算波动率与风险相关特征。
  - **第189-191行**：计算价格位置特征。
  - **第194-199行**：计算基本面变化率特征。
  - **第203-233行**：计算额外的alpha因子，包括反转因子、波动率调整动量、成交额变化趋势、价格加速度、量价背离和上影线比例。
  - **第236-261行**：生成标签，计算T+1日开盘买入到T+1+HOLD_DAYS日开盘卖出的收益率，并进行Winsorize处理。
  - **第264-265行**：将所有特征转为DataFrame并返回。

### 第268-337行：compute_all_features函数
...
def compute_all_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df
```
- **第268-337行**：定义compute_all_features函数，对所有股票批量计算特征。
  - **第269行**：函数文档字符串，说明函数的作用。
  - **第270行**：打印开始特征工程的信息。
  - **第271行**：按股票代码和日期排序并重置索引。
  - **第274行**：打印计算个股技术指标的信息。
  - **第277行**：按股票代码分组。
  - **第280-291行**：遍历每个股票组，计算特征并写入原DataFrame。
  - **第293行**：打印完成所有股票特征计算的信息。
  - **第296行**：打印计算截面特征的信息。
  - **第297-301行**：计算市场环境特征，包括市场平均收益率、标准差和涨跌比。
  - **第304-305行**：计算市场动量（5日滚动）。
  - **第307行**：将市场环境特征合并到原DataFrame。
  - **第310行**：计算个股相对市场的超额收益。
  - **第313行**：打印计算截面排名的信息。
  - **第314-315行**：计算基本面指标的截面排名。
  - **第318-319行**：计算动量指标的截面排名。
  - **第322行**：打印清理NaN行的信息。
  - **第323-326行**：删除特征计算初期的NaN行（前60日）。
  - **第329-334行**：将特征列转换为float32类型以节省内存。
  - **第336行**：打印特征工程完成的信息。
  - **第337行**：返回计算特征后的数据。

### 第340-345行：get_feature_columns函数
...
```
- **第340-345行**：定义get_feature_columns函数，用于获取所有特征列名（排除标识列、标签列等）。

### 第348-395行：run_pipeline函数
...
def run_pipeline(end_date=None):
    ...
    return df
```
- **第348-395行**：定义run_pipeline函数，执行特征工程流水线。
  - **第349-353行**：函数文档字符串，说明函数的作用和参数。
  - **第356-377行**：缓存命中检查，如果缓存文件存在且有效，则直接返回缓存数据。
  - **第379行**：读取清洗后的数据。
  - **第382-384行**：如果指定了截止日期，截断数据。
  - **第386行**：调用compute_all_features函数计算特征。
  - **第389-390行**：创建目录并保存特征数据。
  - **第391-393行**：打印保存信息和特征列信息。
  - **第395行**：返回计算特征后的数据。
...
```
- **第398-405行**：主程序入口，允许通过命令行参数指定数据截止日期。
  - **第399行**：导入sys模块。
  - **第400行**：初始化_end_date为None。
  - **第401行**：获取命令行参数。
  - **第402-404行**：如果命令行参数中包含'--date'，则获取指定的截止日期。
  - **第405行**：调用run_pipeline函数，传入截止日期参数。